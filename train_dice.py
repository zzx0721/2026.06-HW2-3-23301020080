...
def main():
    ...
    wandb.init(project="unet-pet-segmentation", name="run_loss_dice", settings=wandb.Settings(_disable_stats=True))
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"\n========================================")
    print(f"当前使用设备: {device} | 正在运行: 纯 Dice Loss 训练")
    ...
    for epoch in range(epochs):
        ...
        for images, masks in train_pbar:
            ...
            loss = criterion_dice(outputs, masks)  # <--- 固定使用 Dice Loss
            loss.backward()
            optimizer.step()
        ...
        wandb.log({"epoch": epoch + 1, "Train Loss (Dice)": train_loss_avg, "Val Loss (Dice)": val_loss_avg,
                   "Val mIoU (Dice)": val_miou_avg})

    torch.save(model.state_dict(), "unet_dice_weights.pth")
    print("Dice 模型已保存！")
    wandb.finish()
